...
def render_evidence_section(title: str, items: list[dict[str, Any]], risk: bool = False) -> str:
    if not items:
        body = '<p class="muted">暂无经公开证据确认的内容。</p>'
    else:
        rows: list[str] = []
        for item in items:
...
<main>
  ...
  {render_evidence_section('技术路径', data['technical_paths'])}
  {render_evidence_section('同类需求路径', data['peer_paths'])}
  {render_evidence_section('合规风险热区', data['compliance_risks'], True)}
  <section class="panel"><h2>待验证候选与待验证项</h2><h3>待验证候选（不绘制坐标）</h3>{('<ul class="evidence-list">' + pending_rows + '</ul>') if pending_rows else '<p class="muted">暂无。</p>'}<h3 style="margin-top:16px">待验证项</h3>{pending_items}</section>
  <section class="panel"><h2>来源</h2>{('<ul class="evidence-list">' + source_rows + '</ul>') if source_rows else '<p class="muted">暂无外部来源；本报告为待验证地图。</p>'}</section>
